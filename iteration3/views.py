        ...
        if entries.exists():
            ...
            if start and end:
                ...
                if entries:
                    ...
                    df = pd.DataFrame({'x': x, 'Blood Sugar (mmol/L)': bsl, 'Insulin (units)': isl, 'Carbs (g)': carbs})

                    # Melting for faceting purposes.
                    df_melt = df.melt(id_vars='x', value_vars=['Blood Sugar (mmol/L)', 'Insulin (units)', 'Carbs (g)'])
